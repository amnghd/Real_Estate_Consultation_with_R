...
**MS Subclass,  Overal Qual, Overal Cond,  **
...
ames_train %>% group_by(Neighborhood) %>% summarize(standard_dev = sd(price )) %>% arrange(desc(standard_dev))
```
...
p1 <- ggplot(ames_train, aes(x=Lot.Area, y=price)) +geom_point(color = 'magenta') + ggtitle("Lot area relationship")
p2 <- ggplot(ames_train, aes(x=Bedroom.AbvGr, y=price)) +geom_point(color = 'green') + ggtitle("Bedrooms above grade relationship")
p3 <- ggplot(ames_train, aes(x=Overall.Qual, y=price)) +geom_point(color = 'blue') + ggtitle("Overall quality relationship")
p4 <- ggplot(ames_train, aes(x=Year.Built, y=price)) +geom_point(color = 'red') + ggtitle("Years built relationship")
...
ggplot(ames_train, aes(x=log(area), y=log(price))) +geom_point(color = 'magenta') + ggtitle("log-area vs log-price")
```
...
$$\text{Beta}(\alpha = 9 + n,~ \beta = 1 + n - k) $$

```{r Q6}
k <- ames_train$Garage.Type%>%is.na()%>%sum()
n <- nrow(ames_train)  - k
...
after_1999 <- nrow(filter(ames_train, Year.Built > 1999)) / n * 100
median_mean <- ames_train $ price %>% median() / ames_train$price%>%mean()
no_basement <- ames_train$Bsmt.Qual%>%is.na()%>%sum()
...
mean_diff <- ames_train %>% filter(is.na(Garage.Type)) %>%summarise(mean =  mean(area)) - ames_train %>% filter(!is.na(Garage.Type)) %>%summarise(mean =  mean(area))

mean_total <- ames_train %>% summarize(mean = mean(area))

mean_out_data <- ames_train $ area  - 1476.5

SE = sd(mean_out_data)/ sqrt(length(mean_out_data))
...
mean_diff <- ames_train %>% filter(is.na(Garage.Type)) %>%summarise(mean =  mean(area)) 
```
...
```{r}
ames_train <- ames_train %>% mutate(log_price = log(price), log_area = log(area))
...
ggplot(ames_train, aes(x=log_area, y=log_price)) +geom_point(color = 'magenta') +
  ggtitle("log-area vs log-price") + geom_smooth(method='lm') +
  geom_text(aes(label=ifelse((log_area>3*IQR(log_area)|log_price>3*IQR(log_price)),c(Overall.Qual),"")), hjust=1.1)

```

```{r}
sd(ames_train$log_price)
```
...
```{r}
ggplot(ames_train, aes(x=price)) +geom_histogram(color = 'gold')
```
...
a <- ames_train %>% filter(Bldg.Type == '1Fam') %>% group_by(Neighborhood) %>% count()

b <- ames_train %>% group_by(Neighborhood) %>% count()
...
ggplot(ames_train, aes(x=Bedroom.AbvGr, y=log_area)) +geom_jitter(color = 'Navy')
...
filtered <- ames_train %>% filter(!is.na(Total.Bsmt.SF))
filtered $ Total.Bsmt.SF %>% mean()
